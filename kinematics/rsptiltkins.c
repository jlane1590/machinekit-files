...
static const double initialHeight =		3.857;
static const double hornLength =		1.25;
static const double legLength =			4.0;
static const double lambda = 			12.0;	//angle of tilt of servos
		      ...
		      KINEMATICS_FORWARD_FLAGS * fflags)
{
	...
	for(i = 0; i < 6; i++)
	{
		...
		double M = 2*hornLength*(-rtapi_sin(radians(lambda))*rtapi_sin(radians(beta[i]))*(q.x-b[i].x) + rtapi_sin(radians(lambda))*rtapi_cos(radians(beta[i]))*(q.y-b[i].y) + rtapi_cos(radians(lambda))*(q.z-b[i].z));
...
static const char *name = "rsptiltkins";
