...
void do_homing(void)
{
    int joint_num, check_joint_num, joints_ready;
    emcmot_joint_t *joint, *check_joint;
    double offset, tmp;
	...
	do {
		...
		if (home_sw_active) {
		    ...
		    joint->free_tp_enable = 0;
		    /* go to next step */
		    joint->home_state = HOME_WAIT_JOINTS_READY_FINAL_SEARCH;
		    immediate_state = 1;
		    break;
		}
		home_do_moving_checks(joint);
		...
		case HOME_WAIT_JOINTS_READY_FINAL_SEARCH:
		/* this state is called when the joint is waiting for the rest
			of the joints in the sequence number to do the initial switch trip.
			the joint will wait in this state until all joints are ready to perform the next move*/
			
			joints_ready = 1;
			//check all the joints to see if we have to wait on any
			for(check_joint_num = 0; check_joint_num < num_joints; check_joint_num++)
			{
				check_joint = &joints[check_joint_num];
				if(check_joint->home_state != HOME_IDLE && check_joint->home_state < HOME_WAIT_JOINTS_READY_FINAL_SEARCH)
				{
					//this joint is currently homing and it has not reached this point yet so we have to wait.
					joints_ready = 0;
				}
			}
		...
		joint->motor_offset -= offset;
		/* next state */
		joint->home_state = HOME_WAIT_JOINTS_READY_FINAL_MOVE;
		immediate_state = 1;
		break;
		...
		case HOME_WAIT_JOINTS_READY_FINAL_MOVE:
		/* this state is called when the joint is waiting for the rest
			of the joints in the sequence number to get the final offset
			the joint will wait in this state until all joints are ready to perform the next move*/
			...
			{
				check_joint = &joints[check_joint_num];
				if(check_joint->home_state != HOME_IDLE && check_joint->home_state < HOME_WAIT_JOINTS_READY_FINAL_MOVE)
				{
					//this joint is currently homing and it has not reached this point yet so we have to wait.
					joints_ready = 0;
				}
		...
		joint->home_state = HOME_IDLE;
		immediate_state = 1;
		//maybe I can set the carte-pos-cmd and carte-pos-fb to the known home location, then I only set the cartesian position and do invers kins
