		      ...
		      KINEMATICS_FORWARD_FLAGS * fflags)
{
	...
	for(i = 0; i < 6; i++)
	{
		A.x = p[i].x*rtapi_cos(rpy.p) + p[i].y*rtapi_sin(rpy.p)*rtapi_sin(rpy.r) + pos->tran.x - b[i].x;
		A.y = p[i].y*rtapi_cos(rpy.r) + pos->tran.y - b[i].y;
		A.z = rtapi_sqrt(Amag*Amag - A.x*A.x - A.y*A.y);
		
		if(rtapi_isnan(A.z))
			return -1;
		
		joints[i] = pos->tran.z - p[i].x*rtapi_sin(rpy.p) + p[i].y*rtapi_cos(rpy.p)*rtapi_sin(rpy.r) - A.z;
/*		if(joints[i] < 0.0)
			joints[i] = 0.0;
		else if(joints[i] > STROKE)
			joints[i] = STROKE;
*/
	}
